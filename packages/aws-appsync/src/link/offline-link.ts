...
const enqueueMutation = <T>(
  ...
  const optimisticResponse =
    typeof origOptimistic === "function"
      ? origOptimistic(variables)
      : origOptimistic;
  ...
  if (optimisticResponse) {
    ...
    result = mutationDefinition.selectionSet.selections.reduce(
      (acc, elem: FieldNode) => {
...
const effect = async <TCache extends NormalizedCacheObject>(
  ...
  return new Promise((resolve, reject) => {
    ...
    const getObservableFromLink: Function = (client.queryManager as any)
    ...
    const operation = getObservableFromLink.call(
      client.queryManager,
      mutation,
      extraContext,
      variables,
...
const _discard = (error, action: OfflineAction, retries) => {
  const {
    graphQLErrors = [],
  }: { graphQLErrors: AWSAppsyncGraphQLError[] } = error;
  ...
  if (graphQLErrors.length) {
    ...
    const appSyncClientError = graphQLErrors.find(
      (err) => err.errorType && err.errorType.startsWith("AWSAppSyncClient:")
    );
...
const mapIds = (obj1, obj2) =>
  intersectingKeys(obj1, obj2).reduce(
    (acc, k) => ((acc[obj1[k]] = obj2[k]), acc),
    {}
  );
//#endregion
